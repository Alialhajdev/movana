...
export function SeriesCard({ series, className }: Props) {
  ...
  return (
    <Link
      ...
      params={{ id: series.id }}
      className={cn(
        "group relative block w-[180px] sm:w-[210px] md:w-[230px] shrink-0 card-hover",
        className
      )}
    >
      <div
        className={cn(
          "relative aspect-[2/3] overflow-hidden rounded-lg bg-gradient-to-br shadow-card",
          series.posterColor
        )}
      >
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-3 bg-gradient-to-t from-black/95 via-black/30 to-transparent">
          <h3 className="font-display text-lg leading-tight text-white text-shadow-hero line-clamp-2">
            {series.title[lang]}
          </h3>
        ...
        {series.isNew && (
          <span className="absolute top-2 start-2 rounded-md bg-primary px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-primary-foreground">
            New
          </span>
        )}
      </div>
